...
import org.openhab.binding.bluetooth.directbt.internal.metrics.ActorMetricsListener;
...
public class DirectBTBluetoothDevice extends BaseBluetoothDevice implements DevicePort {
    ...
    // Scan-assisted LE connection parameters, tuned for reliability on marginal/weak-RSSI links. Units:
    // 0.625ms for scan, 1.25ms for connection interval, 10ms for supervision. The connection interval and
    // supervision timeout are bridge-configurable; Direct-BT pins min=max to keep controller scheduling
    // predictable.
    ...
    // Per-device connect/read/write service-level metrics, published to the openHAB Micrometer registry. Resolved
    // lazily: a device object exists from its first advertisement, but only devices with a configured Thing are
    // instrumented (see DirectBTBridgeHandler.createDeviceMetrics), and the Thing may be created later.
    private volatile @Nullable DeviceMetrics metrics;
    // When the current connect attempt was issued (System.nanoTime(); 0 = none in flight). Split from the GATT
    // phase below because establishing the link and resolving GATT fail differently and at different timescales.
    private volatile long connectStartedNanos;
    // When the link came up, so the connected -> GATT-resolved phase can be timed on its own.
    private volatile long linkUpNanos;
    ...
    DirectBTBluetoothDevice(DirectBTBridgeHandler adapter, BluetoothAddress address, Clock clock) {
        ...
        this.clock = clock;
        // The control-plane metrics listener resolves the device's metric set lazily (this::metrics), exactly like
        // the instrumentation call sites: a device only gets meters once it has a configured Thing, and the actor
        // can transition before then.
        ...
        }, adapter.getResetBudget(), bridge::requestAdapterReset, clock, new ActorMetricsListener(this::metrics));
    }
    ...
    /**
     * @return this device's metric set, or a no-op one while the device has no configured Thing (only configured
     *         devices are instrumented; see {@link DirectBTBridgeHandler#createDeviceMetrics}). Re-asks the bridge
     *         until it gets one, so a device that gains a Thing after being discovered starts publishing then.
     */
    private DeviceMetrics metrics() {
        DeviceMetrics current = metrics;
        if (current == null) {
            current = bridge.createDeviceMetrics(address);
            if (current == null) {
                return DeviceMetrics.NOOP;
            }
            metrics = current;
        }
        return current;
    ...
    protected void dispose() {
        // Drop this device's meters, otherwise a deleted or aged-out device keeps publishing a frozen series
        // (and a stale "up" value) for the lifetime of the runtime. Called by the bridge for both teardown and
        // the inactive-device cleanup, which removes the device right after. Read the field directly: going
        // through metrics() would REGISTER meters for a device that never had any, on its way out.
        DeviceMetrics current = metrics;
        if (current != null) {
            current.close();
        }
    ...
    public void markConnected() {
        ...
        // One link established. Counted here because this is the single point the controller reports the link
        // up, so it is a true "how many times did this device connect" figure.
        metrics().connected();
        // The link is up: the connect attempt succeeded. The GATT phase starts here and is closed out by
        // resolveGatt(); the device is not counted as "up" until that model is usable.
        long attemptStarted = connectStartedNanos;
        long now = System.nanoTime();
        if (attemptStarted != 0) {
            metrics().operation(Op.CONNECT);
            metrics().record(Op.CONNECT, Phase.LINK, now - attemptStarted);
            connectStartedNanos = 0;
        }
        linkUpNanos = now;
    ...
    // Instrumented here rather than in the public entry points so BOTH disconnect paths (the ordinary one and
    // the adapter-reset one) are counted; an adapter reset that drops a link is still a real outage.
    private void markDisconnected(boolean nativeCleanup) {
        // The device is unavailable the moment the link goes, regardless of what teardown still has to run.
        metrics().setUp(false);
        // A disconnect while a connect attempt is still in flight IS that attempt's failure (a silent drop, or
        // an establishment failure the reconciler timed out on). Without this the attempt would never be
        // counted and the connect success ratio would silently ignore its most common failure mode.
        long attemptStarted = connectStartedNanos;
        if (attemptStarted != 0) {
            metrics().operation(Op.CONNECT);
            metrics().failure(Op.CONNECT, Cause.DISCONNECTED);
            metrics().record(Op.CONNECT, Phase.LINK, System.nanoTime() - attemptStarted);
            connectStartedNanos = 0;
        }
        // Count the dropped link only when one was actually established (linkUpNanos is set by markConnected).
        // A connect attempt that never came up is a failed attempt, not a disconnect, so counting it here would
        // make "reconnects" indistinguishable from "attempts that never worked".
        if (linkUpNanos != 0) {
    ...
    public HCIStatusCode connectNative() {
        ...
        // Time the connect procedure from the attempt, not from the native call: the reconciler counts the whole
        // procedure against its deadline, so that is the latency the availability story depends on.
        connectStartedNanos = started;
        BTDevice dev = device;
        if (dev == null) {
            metrics().operation(Op.CONNECT);
            metrics().failure(Op.CONNECT, Cause.DISCONNECTED);
        ...
        try {
            ...
            // dev.disconnect() residue-clear here was removed and proven
            ...
            // SUCCESS only means the controller ACCEPTED the create-connection; the link is not up yet, so the
            // operation stays in flight and is completed by markConnected()/markDisconnected(). Any other status
            // is an immediate rejection and completes the attempt here.
            if (rc != HCIStatusCode.SUCCESS) {
                metrics().operation(Op.CONNECT);
                metrics().failure(Op.CONNECT,
                        rc == HCIStatusCode.COMMAND_DISALLOWED ? Cause.COMMAND_DISALLOWED : Cause.NATIVE_ERROR);
                metrics().record(Op.CONNECT, Phase.LINK, System.nanoTime() - started);
                connectStartedNanos = 0;
            }
            return rc;
        } catch (RuntimeException e) {
            ...
            metrics().operation(Op.CONNECT);
            metrics().failure(Op.CONNECT, Cause.NATIVE_ERROR);
            metrics().record(Op.CONNECT, Phase.LINK, System.nanoTime() - started);
            connectStartedNanos = 0;
            return HCIStatusCode.INTERNAL_FAILURE;
        }
    ...
    /**
     * Closes out the GATT phase of the connect procedure: counts it, times it from the moment the link came up,
     * and drives the availability gauge — the device only counts as "up" once its GATT model is usable, which is
     * the point from which openHAB can actually read it.
     */
    private void recordGattResolve(boolean resolved, Cause cause) {
        long linkUp = linkUpNanos;
        metrics().operation(Op.CONNECT);
        if (!resolved) {
            metrics().failure(Op.CONNECT, cause);
        }
        if (linkUp != 0) {
            metrics().record(Op.CONNECT, Phase.GATT, System.nanoTime() - linkUp);
        }
        metrics().setUp(resolved);
    }
        ...
        try {
            if (isGattResolved()) {
                logger.debug("Direct-BT GATT discovery for {} skipped: already resolved with {} mapped chars", address,
                        gattChars.size());
                // Already usable — keep the availability gauge true without re-timing a resolve that never ran.
                metrics().setUp(true);
                return true;
            }
            BTDevice dev = device;
            if (dev == null) {
                recordGattResolve(false, Cause.DISCONNECTED);
                return false;
            }
            ...
            // "Resolved" means a usable model: an empty map is a failed resolve, not a fast success.
            recordGattResolve(!gattChars.isEmpty(), Cause.RESOLVE_FAILED);
        } catch (RuntimeException e) {
            logger.debug("Direct-BT service discovery for {} failed after {}ms", address, elapsedMillis(started), e);
            recordGattResolve(false, Cause.NATIVE_ERROR);
            return false;
        } finally {
    ...
    public CompletableFuture<byte[]> readCharacteristic(BluetoothCharacteristic characteristic) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                if (value == null) {
                    // No native value came back: the Direct-BT receive-path-death signature — the peer answered
                    // but nothing reached the stack. Counted separately because it is OUR fault, not the device's.
                ...
                if (!counted) {
                    metrics().failure(Op.READ, Cause.NATIVE_ERROR);
                }
                throw new CompletionException(e);
            } finally {
                metrics().record(Op.READ, Phase.NONE, System.nanoTime() - started);
            }
        }, executor);
    }
    ...
    public CompletableFuture<@Nullable Void> writeCharacteristic(BluetoothCharacteristic characteristic, byte[] value) {
        ...
        return CompletableFuture.runAsync(() -> {
            long started = System.nanoTime();
            metrics().operation(Op.WRITE);
            boolean counted = false;
            try {
                BTGattChar gattChar = connectedChar(characteristic);
                if (gattChar == null) {
                    metrics().failure(Op.WRITE, Cause.DISCONNECTED);
                    counted = true;
                    throw new IllegalStateException("Characteristic not available (disconnected?): " + charUuid);
                }
                ...
                if (!gattChar.writeValue(value, withResponse)) {
                    metrics().failure(Op.WRITE, Cause.WRITE_REJECTED);
                    counted = true;
                    throw new IllegalStateException("writeValue returned false");
                }
            } catch (RuntimeException e) {
                if (!counted) {
                    metrics().failure(Op.WRITE, Cause.NATIVE_ERROR);
                }
                throw new CompletionException(e);
            } finally {
                metrics().record(Op.WRITE, Phase.NONE, System.nanoTime() - started);
            }
        }, executor);
    }
