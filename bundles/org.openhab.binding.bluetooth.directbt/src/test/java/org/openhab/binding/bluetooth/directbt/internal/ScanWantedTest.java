...
/**
 * Regression harness for {@link DirectBTBridgeHandler#scanWanted} — the pure predicate deciding whether the
...
class ScanWantedTest {
    ...
    void idleWithNothingWantedDoesNotScan() {
        assertFalse(DirectBTBridgeHandler.scanWanted(false, false, false, false, false));
    }
    ...
    void configuredDeviceNeedingDiscoveryScans() {
        ...
        assertTrue(DirectBTBridgeHandler.scanWanted(true, false, false, false, false));
        assertTrue(DirectBTBridgeHandler.scanWanted(true, false, false, false, true),
                "a device needing a handle must scan to get one, regardless of another device establishing");
    }

    @Test
    void backgroundDiscoveryScansForTheInboxWhenIdle() {
        assertTrue(DirectBTBridgeHandler.scanWanted(false, true, false, false, false));
    }

    @Test
    void manualScanScansForTheInboxWhenIdle() {
        assertTrue(DirectBTBridgeHandler.scanWanted(false, false, true, false, false));
    }
    ...
    void backgroundDiscoveryYieldsToADeviceEstablishingItsConnection() {
        ...
        // never completes.
        assertFalse(DirectBTBridgeHandler.scanWanted(false, true, false, false, true),
                "background discovery must yield to a device establishing its link");
    }

    @Test
    void manualScanYieldsToADeviceEstablishingItsConnection() {
        assertFalse(DirectBTBridgeHandler.scanWanted(false, false, true, false, true),
    ...
    void anyScanIsSuppressedWhileADeviceIsActivelyConnecting() {
        ...
        assertFalse(DirectBTBridgeHandler.scanWanted(true, true, true, true, true));
    }
}
